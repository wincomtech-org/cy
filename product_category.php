...
if (isset($_GET['id']) && strpos($_SERVER['HTTP_HOST'],'cypro')===false) {
    ...
    if ($cat_id==4 || $cat_id==22) {
        header('Location:http://cypro.wincomtech.cn/product_category.php?id='.$cat_id);exit();
    }
}
...
if ($cat_id == -1) {
    ...
    if (empty($cate_info)) {
        ...
        if ($cat_info['image']) {
            $thumb = explode('.', $cat_info['image']);
            $cat_info['thumb'] = ROOT_URL . $thumb[0] . "_thumb." . $thumb[1];
        } else {
            $cat_info['thumb'] = ROOT_URL .'images/nopic_s_100x100.jpg';
        }
    }
}
...
if ($checkids) {
    if ($check_last) {
        ...
        $sql = sprintf('SELECT %s,b.cat_name from %s as a join %s b on a.cat_id=b.cat_id %s order by id desc %s',$fields,$dou->table('product'),$dou->table('product_category'),$where,$limit);
        $query = $dou->query($sql);
    ...
    $sql = sprintf('SELECT %s from %s %s order by id desc %s',$fields,$dou->table('product'),$where,$limit);
    $query = $dou->query($sql);
