...
// 权限判断
$rbac->access_jump('manager',$_USER);
...
if ($rec == 'default') {
    ...
    $where = $_USER['action_level']==99?'':"WHERE pid={$gUid} OR user_id={$gUid}";
    $sql = "SELECT user_id,user_name,email,action_level,add_time,last_login FROM " . $dou->table('admin') . " {$where} ORDER BY user_id ASC";
    $query = $dou->query($sql);
...
elseif ($rec == 'add') {
    ...
    //     $roles = $dou->fetchAll("SELECT * from ".$dou->table('rbac_role')." where code!='ALL' ");
    // } elseif ($_USER['action_level']==50) {
    //     $roles = $dou->fetchAll("SELECT * from ".$dou->table('rbac_role')." where code!='ALL' and code!='ADMIN' ");
    // }
...
elseif ($rec == 'insert') {
    ...
    $dou->insert('admin',$data);
    $dou->create_admin_log($_LANG['manager_add'] . ': ' . $user_name);
...
elseif ($rec == 'edit') {
    ...
    if ($_USER['action_level']<50 && $manager_info['user_name']!=$_USER['user_name']) {
        $dou->dou_msg($_LANG['without'], 'manager.php');
    }
...
elseif ($rec == 'update') {
    ...
    $dou->update('admin',$data,'user_id='.$id);
...
elseif ($rec == 'del') {
    ...
    if ($user_name==$_USER['user_name'] || ($_USER['action_level']<50 && $manager_info['user_name']!=$_USER['user_name'])) {
