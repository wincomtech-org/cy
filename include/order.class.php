    ...
    */
    ...
    function get_cart($session_cart,$shell='') {
        ...
        if (count($session_cart)>1) {
            ...
            $products = $GLOBALS['dou']->fetchAll("SELECT id,name,model,price,image,defined from ".$GLOBALS['dou']->table('product')." WHERE id IN ({$pro_ids})");
        } else {
            // 硬凑一个二维数组
            $pro_ids = $pro_ids[0];
            $products[0] = $GLOBALS['dou']->fetchRow("SELECT id,name,model,price,image,defined from ".$GLOBALS['dou']->table('product')." WHERE id={$pro_ids}");
        }
        // return $pro_ids;
        // return $products;
