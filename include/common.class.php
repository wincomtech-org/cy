...
class Common extends DbMysql {
    ...
    function get_nav($type = 'middle', $parent_id = 0, $current_module = '', $current_id = '', $current_parent_id = '') {
        $nav = array();
        $data = $this->fetch_array_all($this->table('nav'), 'sort ASC', "lang_id='{$GLOBALS[lang_type]}'");
    ...
    function dou_module() {
        ...
        // 读取模块初始化文件  具体加载顺序：lang.init.php  smarty.init.php  user.init.php
    ...
    function dou_url() {
        ...
        // 会员模块常用URL
        foreach (explode('|', 'login|register|logout|order|order_detail') as $value)
    ...
    function get_category_nolevel($table, $parent_id = 0, $level = 0, $current_id = '', &$category_nolevel = array(), $mark = '-') {
        if(in_array($table,array('district','diy','diy_category'))) {
            $data = $this->fetch_array_all($this->table($table), 'sort,cat_id');
        } else {
            $data = $this->fetch_array_all($this->table($table), 'sort,cat_id', "lang_id='{$GLOBALS[lang_type]}'");
        }
        foreach ((array)$data as $value) {
    ...
    * $cat_id   分类ID
    * $sel      字段
    * $where    条件          多语言： , "land_id='{$GLOBALS[lang_type]}'"
    * $order    排序
    * $limit    调用数量
    * +----------------------------------------------------------
    */
    ...
    function get_page_list($parent_id = 0, $current_id = '') {
        $page_list = array();
        $data = $this->fetch_array_all($this->table('page'), 'id ASC', "lang_id='{$GLOBALS[lang_type]}'");
        foreach ((array) $data as $value) {
            // $parent_id将在嵌套函数中随之变化
            if ($value['parent_id'] == $parent_id) {
                $value['url'] = $this->rewrite_url('page', $value['id']);
    ...
    function pager($table, $page_size=10, $page, $page_url='', $where='', $get='', $join='', $close_rewrite=false, $sep='page') {
        ...
        $page_count = ceil($record_count/$page_size);// 总共的页码
    ...
    */
     ...
     * +----------------------------------------------------------
     */
    function dou_substr($str, $length=26, $clear_space=true, $dot='……', $charset=DOU_CHARSET) {
