...
// error_reporting(E_ALL ^ (E_ERROR|E_PARSE));// E_ERROR：通常会显示出来，也会中断程序执行。E_PARSE：语法解析错误。
...
// echo $GLOBALS['syskey'];die;
// 读取站点信息
$_CFG = $dou->get_config();
$_CFG['lang_type'] = $lang_type ? $lang_type : 1;
...
if (!defined('EXIT_INIT')) {
    ...
    // 企业站 与 产品站 区分
    if (strpos($_SERVER['HTTP_HOST'],'cypro')!==false) {
        $_CFG['cypro'] = true;
        if(strpos($_SERVER['REQUEST_URI'],'.php')==false || strpos($_SERVER['REQUEST_URI'],'index.php')){
            $dou->dou_header($_CFG['domain_pro'] .'?id='.($cat_id?$cat_id:($_CFG['lang_type']==1?4:22)));
        }
    } else {
        $_CFG['cypro'] = false;
    }
