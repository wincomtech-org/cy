...
function ff1(){
    var qw=$("#nkname").val();
    if(qw!=""){ $("#o1").text("填写正确");return true;}
    else{$("#o1").text("不能为空");return false;}
}
function ff2(){
    var qw=$("#rlname").val();
    if(qw!=""){ $("#o2").text("填写正确");return true;}
    else{$("#o2").text("不能为空");return false;}
}
function ff3(){
    var qw=$("#pe").val();
    var Reg = qw.match(telcheck);
    if(Reg==null){ $("#o3").text("请正确填写");return false;}
    else{$("#o3").text("填写正确");return true;}
}
function ff4(){
    var qw=$("#mailb").val();
    var Reg = qw.match(emailcheck);
    if(Reg==null){ $("#o4").text("请正确填写");return false;}
    else{$("#o4").text("填写正确");return true;}
}
function ff5(){
    var qw=$("#contact").val();
    var eng = qw.match(enlnamereg);
    var zh = qw.match(zhnamereg);
    if(eng==null && zh==null){ $("#o4").text("请正确填写");return false;}
    else{$("#o4").text("填写正确");return true;}
}
function send(){
...
function oldpassw(){
    ...
    if(pw>5&&pw<9){ $("#om1").text("填写正确");return true; }
    else{ $("#om1").text("请正确填写");return false; }

};
function newpassw1(){
    var pw=$("#newpw1").val().length;
    if(pw>5&&pw<9){ $("#om2").text("填写正确");return true; }
    else{ $("#om2").text("请正确填写");return false; }
};
function newpassw2(){
    var pw1=$("#newpw1").val();
    var pw2=$("#newpw2").val();
    if(pw1==pw2){
        $("#om3").text("输入一致");return true;
    }else{ 
        $("#om3").text("输入不一致");return false; 
    }
};
function send1(){
